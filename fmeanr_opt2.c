...
void column_sum_masked(float (*image)[SIZE],float (*output)[SIZE], int x_start){
    ...
    float (*float_accumulators)[8] = {a1,a2,a3,a4};
...
void column_sum(float (*image)[SIZE],float (*output)[SIZE], int x_start){
    ...
    float (*accumulators)[8] = {a1,a2,a3,a4};
...
int main() {
    ...
    int x=0;
    for(x; x<=SIZE-VLEN4; x+=(VLEN4-(2*RADIUS))){
        column_sum(image,output,x);
    }
    if(x>SIZE-VLEN4)column_sum_masked(image,output,x);
    normalize(output);
