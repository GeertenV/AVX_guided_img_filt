...
void moving_sum_ver(float (*temp)[SIZE], float (*output)[SIZE]){
    ...
    if(SIZE%VLEN>0){
        ...
        for(int y=1;y<SIZE;y++){
            __m256 sum = _mm256_maskload_ps(&output[y-1][SIZE-(SIZE%VLEN)],mask);
            if(RADIUS+y < SIZE){
                __m256 vector = _mm256_maskload_ps(&temp[y+RADIUS][SIZE-(SIZE%VLEN)],mask);
                sum = _mm256_add_ps(vector, sum);
            }
            if(y>RADIUS){
                __m256 vector = _mm256_maskload_ps(&temp[y-RADIUS-1][SIZE-(SIZE%VLEN)],mask);
                sum = _mm256_sub_ps(sum, vector);
            }
            _mm256_maskstore_ps(&output[y][SIZE-(SIZE%VLEN)],mask,sum);
        }
    }
}
