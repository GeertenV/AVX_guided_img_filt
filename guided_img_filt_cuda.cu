...
__global__ void point_multiply_gpu(float *a,float *b,float *output, int Nx, int Ny, int threadblock_dim, int x_blocks, int y_blocks){
...
__global__ void point_minus_and_multiply_gpu(float *a,float *b,float* c, float *output, int Nx, int Ny, int threadblock_dim, int x_blocks, int y_blocks){
    int block_x = blockIdx.x%x_blocks;
    int block_y = blockIdx.x/x_blocks;  
...
__global__ void calc_a_gpu(float *a,float *b,float eps, float *output, int Nx, int Ny, int threadblock_dim, int x_blocks, int y_blocks){
    int block_x = blockIdx.x%x_blocks;
    int block_y = blockIdx.x/x_blocks;  
...
__global__ void calc_output_gpu(float *a, float *b, float *c, int *output, int Nx, int Ny, int threadblock_dim, int x_blocks, int y_blocks){
    int block_x = blockIdx.x%x_blocks;
    int block_y = blockIdx.x/x_blocks;  
...
int main() {
    ...
    for(int channel = 0; channel < 3 ; channel++){
        ...
        point_multiply_gpu<<<x_blocks*y_blocks, threadblock_dim*threadblock_dim>>>(d_i, d_g, d_ig, Nx, Ny, threadblock_dim, x_blocks, y_blocks);
        point_multiply_gpu<<<x_blocks*y_blocks, threadblock_dim*threadblock_dim>>>(d_g, d_g, d_gg, Nx, Ny, threadblock_dim, x_blocks, y_blocks);

        cudaDeviceSynchronize();

        fmeanr_gpu<<<x_blocks*y_blocks, threadblock_dim*threadblock_dim>>>(d_i, d_i_mean, Nx, Ny, threadblock_dim, x_blocks, y_blocks, r);
        fmeanr_gpu<<<x_blocks*y_blocks, threadblock_dim*threadblock_dim>>>(d_g, d_g_mean, Nx, Ny, threadblock_dim, x_blocks, y_blocks, r);
        fmeanr_gpu<<<x_blocks*y_blocks, threadblock_dim*threadblock_dim>>>(d_gg, d_g_corr, Nx, Ny, threadblock_dim, x_blocks, y_blocks, r);
        fmeanr_gpu<<<x_blocks*y_blocks, threadblock_dim*threadblock_dim>>>(d_ig, d_ig_corr, Nx, Ny, threadblock_dim, x_blocks, y_blocks, r);

        cudaDeviceSynchronize();

        point_minus_and_multiply_gpu<<<x_blocks*y_blocks, threadblock_dim*threadblock_dim>>>(d_g_corr, d_g_mean, d_g_mean, d_g_var, Nx, Ny, threadblock_dim, x_blocks, y_blocks);
        point_minus_and_multiply_gpu<<<x_blocks*y_blocks, threadblock_dim*threadblock_dim>>>(d_ig_corr, d_i_mean, d_g_mean, d_ig_cov, Nx, Ny, threadblock_dim, x_blocks, y_blocks);

        cudaDeviceSynchronize();

        calc_a_gpu<<<x_blocks*y_blocks, threadblock_dim*threadblock_dim>>>(d_ig_cov, d_g_var, eps, d_a, Nx, Ny, threadblock_dim, x_blocks, y_blocks);

        cudaDeviceSynchronize();

        point_minus_and_multiply_gpu<<<x_blocks*y_blocks, threadblock_dim*threadblock_dim>>>(d_i_mean, d_a, d_g_mean, d_b, Nx, Ny, threadblock_dim, x_blocks, y_blocks);

        cudaDeviceSynchronize();

        fmeanr_gpu<<<x_blocks*y_blocks, threadblock_dim*threadblock_dim>>>(d_a, d_a_mean, Nx, Ny, threadblock_dim, x_blocks, y_blocks, r);
        fmeanr_gpu<<<x_blocks*y_blocks, threadblock_dim*threadblock_dim>>>(d_b, d_b_mean, Nx, Ny, threadblock_dim, x_blocks, y_blocks, r);

        cudaDeviceSynchronize();

        calc_output_gpu<<<x_blocks*y_blocks, threadblock_dim*threadblock_dim>>>(d_a_mean, d_g, d_b_mean, d_output, Nx, Ny, threadblock_dim, x_blocks, y_blocks);
