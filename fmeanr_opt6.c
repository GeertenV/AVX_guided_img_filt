...
void extract_output_sum_left(float (*output)[SIZE], float (*accumulators)[8], int y, int h){
    int w = 1+RADIUS;
    float moving_sum = 0.0;
    for(int i = 0; i <= RADIUS; i++){
        moving_sum += accumulators[i/VLEN][i%VLEN];
    }
    output[y][0] = moving_sum/(h*w);

    for(int i = 1; i < RADIUS; i++){
        moving_sum += accumulators[(i+RADIUS)/VLEN][(i+RADIUS)%VLEN];
        output[y][i] = moving_sum/(h*(w+i));
    }
    w = 1+RADIUS+RADIUS;
    for(int i = RADIUS; i < VLEN5-RADIUS; i++){
        moving_sum += accumulators[(i+RADIUS)/VLEN][(i+RADIUS)%VLEN];
        moving_sum -= accumulators[(i-RADIUS-1)/VLEN][(i-RADIUS-1)%VLEN];
        output[y][i] = moving_sum/(h*w);
    }
}

void extract_output_sum_middle(float (*output)[SIZE], float (*accumulators)[8], int x_start, int y, int h){
    int w = 1+RADIUS+RADIUS;
    float moving_sum = 0.0;
    for(int i = 0; i <= RADIUS; i++){
        moving_sum += accumulators[i/VLEN][i%VLEN];
    }
    ...
    output[y][RADIUS+x_start] = moving_sum/(h*w);
    for(int i = RADIUS+1; i < VLEN5-RADIUS; i++){
        moving_sum += accumulators[(i+RADIUS)/VLEN][(i+RADIUS)%VLEN];
        moving_sum -= accumulators[(i-RADIUS-1)/VLEN][(i-RADIUS-1)%VLEN];
        output[y][i+x_start] = moving_sum/(h*w);
    }
}

void extract_output_sum_right(float (*output)[SIZE], float (*accumulators)[8], int x_start, int y, int h){
    int w = 1+RADIUS+RADIUS;
...
void column_sum_masked(float (*image)[SIZE],float (*output)[SIZE], int x_start){
    ...
    extract_output_sum_right(output,float_accumulators,x_start,0,1+RADIUS);
    ...
    for(int y = 1; y<RADIUS; y++){
        ...
        accumulators[complete_vectors] = _mm256_add_ps(vector, accumulators[complete_vectors]);
        extract_output_sum_right(output,float_accumulators,x_start,y,1+RADIUS+y);
    }
    ...
    for(int y = RADIUS; y < SIZE - RADIUS; y++){
        ...
        __m256 vector1 = _mm256_maskload_ps(&image[y-RADIUS-1][x_start+(complete_vectors*VLEN)],mask);
        accumulators[complete_vectors] = _mm256_sub_ps(accumulators[complete_vectors], vector1);

        extract_output_sum_right(output,float_accumulators,x_start,y,1+RADIUS+RADIUS);
    }
    ...
    for(int y = SIZE - RADIUS; y<SIZE; y++){
        ...
        extract_output_sum_right(output,float_accumulators,x_start,y,1+RADIUS+(SIZE-1-y));
    }
}
...
void column_sum(float (*image)[SIZE],float (*output)[SIZE], int x_start){
    ...
    if(x_start == 0)extract_output_sum_left(output,accumulators,0,1+RADIUS);
    else if(x_start+VLEN5 >= SIZE) extract_output_sum_right(output,accumulators,x_start,0,1+RADIUS);
    else extract_output_sum_middle(output,accumulators,x_start,0,1+RADIUS);
    ...
    for(int y = 1; y<RADIUS; y++){
        ...
        if(x_start == 0)extract_output_sum_left(output,accumulators,y,1+RADIUS+y);
        else if(x_start+VLEN5 >= SIZE) extract_output_sum_right(output,accumulators,x_start,y,1+RADIUS+y);
        else extract_output_sum_middle(output,accumulators,x_start,y,1+RADIUS+y);
    }
    ...
    for(int y = RADIUS; y < SIZE - RADIUS; y++){
        ...
        __m256 vector6 = _mm256_loadu_ps(&image[y-RADIUS-1][x_start]);
        __m256 vector7 = _mm256_loadu_ps(&image[y-RADIUS-1][x_start+VLEN]);
        __m256 vector8 = _mm256_loadu_ps(&image[y-RADIUS-1][x_start+(2*VLEN)]);
        __m256 vector9 = _mm256_loadu_ps(&image[y-RADIUS-1][x_start+(3*VLEN)]);
        __m256 vector0 = _mm256_loadu_ps(&image[y-RADIUS-1][x_start+(4*VLEN)]);
        accumulator1 = _mm256_sub_ps(accumulator1, vector6);
        accumulator2 = _mm256_sub_ps(accumulator2, vector7);
        accumulator3 = _mm256_sub_ps(accumulator3, vector8);
        accumulator4 = _mm256_sub_ps(accumulator4, vector9);
        accumulator5 = _mm256_sub_ps(accumulator5, vector0);

        if(x_start == 0)extract_output_sum_left(output,accumulators,y,1+RADIUS+RADIUS);
        else if(x_start+VLEN5 >= SIZE) extract_output_sum_right(output,accumulators,x_start,y,1+RADIUS+RADIUS);
        else extract_output_sum_middle(output,accumulators,x_start,y,1+RADIUS+RADIUS);
    }
    ...
    for(int y = SIZE - RADIUS; y<SIZE; y++){
        ...
        if(x_start == 0)extract_output_sum_left(output,accumulators,y,1+RADIUS+(SIZE-1-y));
        else if(x_start+VLEN5 >= SIZE) extract_output_sum_right(output,accumulators,x_start,y,1+RADIUS+(SIZE-1-y));
        else extract_output_sum_middle(output,accumulators,x_start,y,1+RADIUS+(SIZE-1-y));
    }
}
